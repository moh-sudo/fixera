...
// Blocking overlay shown whenever the logged-in user's accepted Terms/Privacy
// version is out of date (or missing). Fires app-wide via Layout.jsx so it
// catches the user regardless of which page they land on after login.
export default function TermsReacceptanceModal() {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 2000, background: 'rgba(10,22,40,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: 'Inter, sans-serif' }}>
      <motion.div initial={{ opacity: 0, y: 24, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.3, ease: 'easeOut' }}
        ...
        <div style={{ color: CL.muted, fontSize: 13.5, lineHeight: 1.6, marginBottom: 20 }}>
          {isFirstTime
            ? 'Please review and accept our Terms of Service and Privacy Policy to continue using Fixera.'
            : 'Our Terms of Service and/or Privacy Policy have changed since you last accepted them. Please review and re-accept to keep using Fixera.'}
        </div>
        ...
        <div onClick={() => setChecked(c => !c)} style={{ display: 'flex', gap: 10, cursor: 'pointer', alignItems: 'flex-start', marginBottom: 20 }}>
          ...
          <span style={{ color: CL.text, fontSize: 13, lineHeight: 1.6 }}>
            I have read and accept the Fixera Terms of Service and Privacy Policy.
          </span>
        </div>
