// ============================================================
//  Fixera Partner M-Pesa Top-Up Status Query — Vercel Serverless
//  Polls Daraja to get the current state of an STK push.
// ============================================================
...
export default async function handler(req, res) {
  ...
  const allowed = await rateLimit(req, res, auth.supabase, `topup-status:${auth.user.id}`, { max: 20, windowSeconds: 60 });
  if (!allowed) return;
  ...
  try {
    ...
    if (rc === '1037') return res.status(200).json({ status: 'timeout',   message: 'Timed out waiting for input' });

    return res.status(200).json({ status: 'failed', message: data.ResultDesc || 'Payment failed' });
  } catch (err) {
    console.error('[mpesa-topup-status]', err.message);
