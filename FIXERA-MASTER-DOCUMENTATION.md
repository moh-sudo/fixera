...
- **Not yet redesigned** (still old SB-Admin-2/emoji style): DisputeCenter (separate from DisputesSection — the `dispute_center` nav id), PartnerEarnings, TaxReport, ServiceAreas, RevenueForecast, PartnerAvailability, and **Dispatch** (~566 lines — the single largest remaining section, not yet started).
