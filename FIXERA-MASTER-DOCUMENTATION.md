...
- **Not yet redesigned** (still old SB-Admin-2/emoji style): Users, Jobs, Analytics, ProductApprovals, Announcements, Quotations, Orders, Notifications, Security, Reviews, Marketing, ServicesCatalog, Content, AlertsFeed, FraudRisk, DisputeCenter (separate from DisputesSection — the `dispute_center` nav id), Dispatch, PartnerEarnings, TaxReport, ServiceAreas, RevenueForecast, PartnerAvailability. These inherit some polish for free via the shared-classes restyle in `admin.css` but haven't had a structural/emoji pass. **Two full nav groups (Partners, Finance) plus Support Center are now fully modernized.** Remaining groups: Content, Reports, Platform, plus standalone Main-group items (Users, Jobs/Orders, Notifications).
