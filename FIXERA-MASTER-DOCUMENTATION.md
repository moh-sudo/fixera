...
- User feedback (July 7, 2026): confirmed only All-Partners/Verification-Queue/Partner-Performance had visibly changed under the Partners nav group — Vendors/Suppliers/Movers/Workforce above were still old-style; addressed in this pass. **Riders and Water Carriers remain old-style** (`RiderOpsSection` ~480 lines, `WaterOpsSection` ~135 lines — large dispatch-style sections, next in line).
- **Not yet redesigned** (still old SB-Admin-2/emoji style): Users, Jobs, Payments, Analytics, ProductApprovals, Payouts, Announcements, Quotations, Orders, Notifications, RiderOps, WaterOps, Security, Reviews, Marketing, ServicesCatalog, Content, Wallets, AlertsFeed, VerificationQueue, RefundManagement, PartnerPerformance, Reconciliation, FraudRisk, DisputeCenter (separate from DisputesSection — the `dispute_center` nav id), Dispatch, PartnerEarnings, TaxReport, ServiceAreas, RevenueForecast, PartnerAvailability. These inherit some polish for free via the shared-classes restyle in `admin.css` but haven't had a structural/emoji pass. Next candidates by traffic: Riders, Water Carriers, Verification Queue, Payments.
