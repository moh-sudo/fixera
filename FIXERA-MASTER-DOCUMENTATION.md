...
### Supabase Auth email = the biggest remaining risk (identified, not yet fixed)
Confirmed in the Supabase dashboard that **Custom SMTP is OFF** — signup-confirmation / password-reset / magic-link emails all use Supabase's **built-in** service, which Supabase explicitly says is **not for production** (throttled to a few emails/hour, sends from a `supabase.co` address). At launch this silently breaks new-user signups. Fix (planned, not done): Supabase → Authentication → SMTP → enable Custom SMTP pointing at Resend (`smtp.resend.com:465`, user `resend`, pass = Resend API key, sender `noreply@fixera.africa`).
