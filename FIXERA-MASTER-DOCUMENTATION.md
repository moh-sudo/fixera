...
User brief: make the admin a true **Operations Command Center** ("what needs my attention right now?"), grouped/collapsible low-scroll sidebar, premium modern-SaaS card look (reference image provided — adapted to Fixera navy+gold, not the reference's purple). Phase 1 shipped: rewrote `web/src/pages/admin/admin.css` into a design system (Inter font, light `#F5F7FB` canvas, rounded 16px cards, soft shadows, navy/gold tokens) which restyles every section's shared classes at once; rebuilt the **shell** in `AdminDashboard.jsx` — white grouped/collapsible sidebar (framer-motion height animation) with Lucide icons (all emoji removed), active = navy pill with gold icon, bottom profile card; regrouped nav into Operations / Support / Partners / Finance / Content / Reports / Platform (section ids unchanged so ROLE_ACCESS + SECTIONS still map); new topbar with section title, global search (Cmd+K), alerts/mail/help buttons, profile dropdown. Also fixed the profiles RLS recursion + admin login (see below). **Phase 2 shipped** — command-center `OverviewSection`: time-aware greeting; a **"Needs your attention"** priority strip (pending verifications, open support tickets, pending refunds, active jobs — each clickable, jumps via the `fixera-nav` CustomEvent; only surfaces counts > 0, else "All clear"); 6 KPI cards with **real-data sparklines** (7-day `daySeries` buckets → recharts Area); partner-breakdown **donut** with center total; recent bookings + 7-day bookings bar chart; also pulls `support_tickets` + `refunds` (graceful if absent); framer-motion stagger. Follow-up per user feedback (July 7, 2026): attention cards now **pulse** (`attn-pulse` CSS animation) so they visually demand attention; **Partner Breakdown always lists all 6 roles** with count + % (not just roles with data — user wanted the full mix visible, always); added a **7-day Revenue area chart** next to Bookings; added a **Live Operations map preview card** (dark, animated live-dot, links to the full `LiveOpsMap`) since the user wanted a map on the overview — the full map is a separate dark full-page Leaflet component, not embeddable as a small card, so this is a preview tile that opens it. Added honest **Website Visits / App Installs** placeholder cards linking out to Google Analytics — explicitly did NOT fabricate numbers; wiring real GA Data API numbers into the dashboard is a follow-up (needs a Google service-account key). Added **real Fixera logo mark** in the sidebar (was a generic Wrench icon) using the existing `/logo-mark-sm.png` crop. Added **auto night mode** matching the customer app's `useCL()` pattern exactly — dark 7pm–6am, resets to light at 6am, no manual toggle (user asked for automatic). Implemented as a `data-theme="dark"|"light"` attribute on `.admin-shell` driven by `useAdminNightMode()` (60s interval check), with a full dark CSS-variable override block in `admin.css` (since every admin color already reads a CSS var, this single block reskins the whole dashboard) plus explicit dark overrides for the handful of hardcoded hex colors (search bar, badges, table headers, sidebar hover). **Phase 3 started (July 7, 2026)** — rolling the design language into the heaviest sub-sections. Given ~36 total section components, scoped to the two highest-traffic ones plus shared primitives (which lift many other sections for free since they already use the shared classes):
- **Shared primitives upgraded globally**: `RoleBadge` now renders a Lucide icon (`ROLE_ICON_COMP` + new `RoleIcon` helper) instead of emoji text; `StatCard` restyled to the card design system AND auto-maps common emoji icon props to Lucide via `EMOJI_ICON_MAP` (so older call sites in Payouts/Quotations/Wallets get clean icons with zero edits to those sections); `PageHeader` gained a motion entrance + optional action slot; `InfoRow` now uses CSS vars (dark-mode correct); `Spinner` de-Bootstrapped.
- **`PartnersSection` fully redesigned** (list + detail view) — no emoji, Lucide role icons/filters/actions, motion entrance.
- **`DisputesSection` (Support Center) fully redesigned** — department/priority Lucide icons, urgent-open tickets get the `attn-pulse` glow (reused from the Overview attention strip), notes thread/assign UI restyled; all ticket logic (advanceStatus, SLA calc, assign, internal notes) unchanged.
- Note: `ChartModal` (in the old Overview) is now dead code — no longer invoked since Phase 2's new KPI cards don't open it. Left in place, low priority cleanup.
- **`PartnerTypeSection` redesigned** — this one component backs 3 nav items at once (Vendors/Suppliers/Movers): now takes a Lucide `Icon` prop instead of an emoji string; call sites in `SECTIONS` updated (`Store`/`Package`/`Truck`).
- **`WorkforceSection` (Crew & Vehicles directory) redesigned** — tab/filter icons, avatar fallback, team-lead star, insurance shield icon, verify/unverify buttons, all on design tokens.
- User feedback (July 7, 2026): confirmed only All-Partners/Verification-Queue/Partner-Performance had visibly changed under the Partners nav group — Vendors/Suppliers/Movers/Workforce above were still old-style; addressed. Then completed the group fully:
- **`RiderOpsSection` fully redesigned** (~480 lines, all 4 tabs: Dispatch, All Legs, Rider Queue, Riders roster) — `LEG_STATUS` now maps to Lucide icons, KPI bar → `stat-card`, assign-rider panel, rider queue summary/active/completed legs all on design tokens. All dispatch logic (assignRider/unassignRider/realtime subscription) unchanged.
- **`WaterOpsSection` fully redesigned** (~135 lines, Water Orders + Carriers tabs) — `ORDER_STATUS` maps to Lucide icons, KPI cards, order table restyled.
- **✅ Partners nav group now 100% complete**: All Partners, Verification Queue, Partner Performance, Vendors, Suppliers, Movers, Riders, Water Carriers, Workforce all on the new design system.
- **Finance group redesigned**: `PaymentsSection` (KPI row, payments/receipts tabs, filters), `PayoutsSection` (status cards, request cards, approve/reject/mark-paid flow), `RefundManagementSection` (ticket list + detail review panel), `ReconciliationSection` (30-day revenue-vs-payouts chart + daily table, chart colors updated to the new palette) — all fully redesigned, no emoji, logic unchanged.
- **`WalletsSection` fully redesigned** (~515 lines, the largest single section in the file) — live wallet alerts, KPI strip, wallets/deposits tabs, partner list, wallet-adjustment form + transaction history, deposit action forms (received/refund/forfeit) + deposit history. All logic, real-time subscription (`admin-wallet-monitor` channel), and wallet/deposit service calls unchanged.
- **✅ Finance nav group now 100% complete**: Payments, Payouts, Wallets, Refund Management, Reconciliation all on the new design system.
- **Broad "yes finish it" sweep (July 21, 2026)** — user gave the explicit mandate to complete the full admin redesign across every remaining section. Redesigned in this batch: `JobsSection`, `ReviewsSection` (new `Stars({n})` filled/outline Lucide-star component), `ProductApprovalsSection`, `MarketingSection` (promo codes), `ServicesCatalogSection` (categories/services + 2 modals), `ContentSection` (banners/FAQ + modal), `UsersSection`, `JobModal` (booking management slide-in — `STATUS_FLOW` now carries an `Icon` field), `QuotationsSection`, `AnalyticsSection` (all recharts colors/styling moved to the design tokens), `AnnouncementsSection` + `AnnouncementCard`, and `OrdersSection` (KPI row, type/status filters, order list table, detail panel — `TYPE_ICON`/`STATUS_NEXT` converted from emoji strings to Lucide component refs). All business logic (status transitions, approve/reject flows, real-time subscriptions, service calls) left untouched — this was purely a visual pass to the shared `var(--*)` token system, matching the treatment already given to Partners/Finance/Support Center.
- **Continued sweep (July 21, 2026)**: redesigned `NotificationsSection` (Broadcasts + Support Tickets tabs), `SecuritySection` (Audit Log + Admin Team tabs), `AlertsFeedSection` (urgent tickets / stuck bookings / pending verifications realtime feed — pulsing red dot on the urgent-tickets heading reuses the `attn-pulse` treatment), `VerificationQueueSection` (document review split-panel), `FraudRiskSection` (safety incidents / repeat claimants / suspended partners) — all converted from emoji/hardcoded-hex to Lucide icons + `var(--*)` design tokens, zero logic changes.
- Redesigned `DisputeCenterSection` (separate from `DisputesSection` — the `dispute_center` nav id, two-sided ruling workflow with customer/partner statement panels, new-dispute modal) — Lucide `Scale` icon, all ruling-option/status colors moved to design tokens.
- Redesigned `PartnerEarningsSection` (per-partner earnings statement, monthly bar chart), `TaxReportSection` (VAT/commission KPI cards + monthly breakdown chart + detail table), `ServiceAreasSection` (~293 lines — metro/county collapsible groups, phase badges, readiness bars, active/inactive toggle switches, bulk activate/pause).
- Redesigned `RevenueForecastSection` (weekly actual/target/forecast line chart + breakdown table) and `PartnerAvailabilitySection` (live online/offline grid per role + coverage-by-area table, 30s auto-refresh).
- **`DispatchSection` redesigned (~566 lines, the largest single section in the file)** — all 5 tabs (Queue, Active Jobs, Delayed, Workforce, Analytics) plus the shared `PartnerPicker` assign/reassign panel converted from emoji/Bootstrap to Lucide icons + `var(--*)` design tokens. Logic untouched (realtime `dispatch-rt` channel subscription, `assignPartner`, delayed-job calculation).
- **🎉 ADMIN DASHBOARD REDESIGN COMPLETE (July 21, 2026)** — every one of the ~36 admin sections across all nav groups (Operations, Support, Partners, Finance, Content, Reports, Platform) now runs on the "Premium Command Center" design system: Inter font, navy/gold tokens, Lucide icons throughout (zero emoji remaining in admin UI), dark-mode-correct via CSS vars, shared `StatCard`/`RoleBadge`/`PageHeader`/`InfoRow`/`Spinner` primitives. All business logic, realtime subscriptions, and service calls were left untouched throughout — this was a pure visual pass, done in ~20 incremental build→deploy→commit checkpoints to keep risk low across the very large file (~8000+ lines). **Cleanup:** deleted the dead `ChartModal` component (~190 lines, unused since Phase 2's Overview rewrite) and its unused `CHART_COLORS` constant from `AdminDashboard.jsx` — no call sites referenced it anywhere in the file.

### Gap-scan pass (July 21, 2026) — mopping up what the section-by-section sweep missed
Ran a full-file scan for leftover emoji, hardcoded hex, and Bootstrap `btn/row/col-md/text-gray/card-body` classes to catch anything the section-by-section sweep skipped. Found and fixed:
- **`TeamManagementSection`** — the entire Team/Agents screen (add-agent form + roster table) had never been touched by the redesign; fully converted to the design-token system.
- **`ServiceDetailsView`, `PartnerCrewPanel`, `PartnerFleetPanel`** — inline sub-panels rendered inside partner detail views (crew roster, vehicle fleet with insurance-expiry badges) — still had raw emoji/hex and Bootstrap buttons; redesigned.
- **`WorkforceSection`** — a duplicate `insBadge` insurance-color helper still had hardcoded hex; converted to tokens.
- **`PartnerPerformanceSection`** — ratings/completion/cancellation bars were on hardcoded hex; converted, plus wrapped the table in the standard `admin-card`.
- **`WalletsSection`** — `DEPOSIT_STATUS_META` and 4 KPI `StatCard`s (`🔒`/`💰`/`🛡️`/`⚠️`) were passing raw emoji strings not covered by the `EMOJI_ICON_MAP` auto-converter, so they were rendering as literal emoji; swapped to explicit Lucide elements. Same fix applied to leftover `StatCard` emoji in `PayoutsSection`, `AnalyticsSection`, and `ReconciliationSection`.
- **Modal forms** inside `AnnouncementsSection`, `MarketingSection` (promo codes), and `ContentSection` (banners/FAQ) — the section shells were redone earlier but these specific create/edit modals still used Bootstrap `row`/`col-md-*` grids and `text-gray-600` labels; converted to CSS-grid + token-styled labels.
- **`DispatchSection`** — one leftover literal `#48BB78` on the "Live Map" button, swapped for `var(--green)`.
- Removed the now-fully-dead legacy `ROLE_ICON` emoji map (`AdminDashboard.jsx`) — the last call site was cleaned up in this pass, `ROLE_ICON_COMP` (Lucide) is the only role-icon source now.
- **Left intentionally as-is**: `TYPE_META`/`BLANK_CAT.icon`/`BLANK_BANNER.emoji` default emoji values (these are user-editable *content* fields — category icons, banner emoji — not UI chrome, and `TYPE_META` is shared with non-admin apps); toast/flash `✓` checkmarks (cosmetic text, not chrome); a couple of deliberate `#36b9cc` accent colors used as a secondary metric color not in the token palette.

### Bug fix: duplicate Fixera logo showing on desktop Sign Up page (July 21, 2026)
User reported seeing an extra Fixera logo "lagging"/floating above the sign-in and sign-up forms when clicking through from the marketing site to "Book Services." Root cause found in `web/src/pages/auth/SignUpPage.jsx`: the mobile-only logo wrapper (`.auth-form-logo`, meant to be `display:none` on desktop per `AuthLayout.jsx`'s CSS and only shown under `900px`) had an **inline** `style={{ display: 'flex', ... }}` — inline styles always beat a class's media-query rule, so the "mobile only" `BrandLogo` was rendering on every screen size, duplicating the logo already shown on the desktop photo panel. `LoginPage.jsx` didn't have this bug (no inline `display`). Fix: removed the inline `display`/`justifyContent` override from `SignUpPage.jsx`, and moved the centering into `AuthLayout.jsx`'s own mobile media-query rule (`.auth-form-logo { display:flex; justify-content:center; }` under `max-width:900px`) so it's centralized instead of copy-pasted per page. Verified via computed-style check: `display:none` at 1280px, `display:flex; justify-content:center` at 375px. Checked the partner app (`worker/src/pages/auth/LoginPage.jsx`) for the same pattern — it was already correct (no inline override), so this was scoped to the customer web app's signup page only.

### Live Ops map switched from dark to light (July 21, 2026)
User asked why the full-page Live Operations map (`web/src/components/LiveOpsMap.jsx`) and its Overview preview tile were permanently dark instead of matching the rest of the admin's light theme — it had been a deliberate "ops control room" choice made during Phase 2 but never revisited. Given the choice between auto day/night matching, always-light, or keeping it dark, **user chose always-light**. Rewrote `LiveOpsMap.jsx`: swapped the CARTO `dark_all` raster tile source for `light_all`, converted the stats sidebar / legend / popups / loading overlay from dark hex (`#0A0E1A`, `#111827`, etc.) to light equivalents matching the admin's light palette (white surfaces, `#F5F7FB` canvas, `#EDF0F5` borders), and brightened role/status marker colors slightly for contrast on a light basemap. Also reworked the Overview page's map-preview tile (`OverviewSection`, `AdminDashboard.jsx`) from a dark navy gradient card to a light gradient card on `var(--*)` tokens so the teaser matches what it opens into. The full-page map is still a fixed light look (not day/night auto-switching) — a deliberate choice per the user, distinct from the rest of the admin shell which does auto-switch.
