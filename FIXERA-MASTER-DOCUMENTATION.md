...
- **Left intentionally as-is**: `TYPE_META`/`BLANK_CAT.icon`/`BLANK_BANNER.emoji` default emoji values (these are user-editable *content* fields — category icons, banner emoji — not UI chrome, and `TYPE_META` is shared with non-admin apps); the Overview page's dark "Live Operations" map-preview tile (permanently-dark card, intentionally uses literal light colors for contrast, not theme tokens); toast/flash `✓` checkmarks (cosmetic text, not chrome); a couple of deliberate `#36b9cc` accent colors used as a secondary metric color not in the token palette.
