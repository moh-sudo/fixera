...
- **Not yet redesigned** (still old SB-Admin-2/emoji style): Users, Jobs, Payments, Analytics, Workforce, ProductApprovals, Payouts, Announcements, Quotations, Orders, Notifications, RiderOps, WaterOps, Security, Reviews, Marketing, ServicesCatalog, Content, Wallets, AlertsFeed, VerificationQueue, RefundManagement, PartnerPerformance, Reconciliation, FraudRisk, DisputeCenter (separate from DisputesSection — the `dispute_center` nav id), Dispatch, PartnerEarnings, TaxReport, ServiceAreas, RevenueForecast, PartnerAvailability. These inherit some polish for free via the shared-classes restyle in `admin.css` but haven't had a structural/emoji pass. Next candidates by traffic: Verification Queue, Payments, Finance group.
